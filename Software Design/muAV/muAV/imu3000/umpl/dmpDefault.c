 ...
 */
/***************************************************************************** *
 * $Id: dmpDefault.c 5705 2011-06-28 19:32:29Z nroyer $ 
 ******************************************************************************/
...
const unsigned char dmpMemory[DMP_CODE_SIZE] = {
...
#ifdef DMPDEFAULT_UNSORTEDKEYLOOKUP
/* DMPDEFAULT_UNSOTEDKEYLOOKUP uses O(n) looks up keys from unsorted list
 * to avoid allocating ram for sorted list */
static unsigned short inv_setup_dmpGetAddress( unsigned short key )
{
    int kk;
    if ( key >= NUM_LOCAL_KEYS ) // Key not in range
        return 0xffff;

    for (kk = 0; kk < NUM_LOCAL_KEYS; ++kk){
        if (key == dmpTConfig[kk].key)
            return dmpTConfig[kk].addr; // Found key, return addr
    }

    return 0xfffe; // Key not found
}
#else
